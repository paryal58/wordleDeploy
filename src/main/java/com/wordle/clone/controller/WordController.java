package com.wordle.clone.controller;

import com.wordle.clone.model.Player;
import com.wordle.clone.repository.PlayerRepository;
...
public class WordController {
    ...
    public Player savePlayer(@RequestBody Player player) {
        Optional<Player> existing = playerRepository.findByName(player.getName());
    ...
    @PutMapping("/player/{name}/increment")
    public Player incrementScore(@PathVariable String name) {
        Player player = playerRepository.findByName(name).orElse(new Player(name, 0));
